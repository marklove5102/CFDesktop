...
| P1 常用控件（12个） | 0% | 🟡 桌面 Shell UI 组件依赖 |
...
| P1-4 | TabBar / TabView | 设置分类导航 |
| P1-5 | ToolBar / ToolButton | 文件管理器工具栏 |
| P1-6 | ComboBox | 设置下拉选项 |
| P1-7 | MenuBar / ContextMenu | 右键菜单/应用菜单 |
| P1-8 | StatusBar | 文件管理器状态栏 |
| P1-9 | ToggleButton | 控制中心快捷开关 |

每个控件需：
- 符合 Material Design 3 规范
- 与 ThemeStyleManager 对接（iOS/Windows 风格自动适配形态）
- HWTier 联动（动效降级策略）
- 单元测试覆盖
